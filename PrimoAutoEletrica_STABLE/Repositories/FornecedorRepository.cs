...
namespace PrimoAutoEletrica.Repositories
{
    public sealed class FornecedorRepository : IFornecedorRepository
    {
        ...
        public FornecedorRepository(Func<DbConnection> connectionFactory, LoggerService logger)
        {
        ...
        public void Inserir(Fornecedor fornecedor)
        {
            ...
            try
            {
                ...
                AddFornecedorParameters(command, connection, fornecedor);
            ...
            try
            {
                ...
                command.CommandText = @"
                    ...
                    SET
                        ...
                        PrazoMedioPagamentoDias = @PrazoMedioPagamentoDias,
                ...
                if (linhasAfetadas == 0)
                {
                    throw new InvalidOperationException("Fornecedor nao encontrado para atualizacao.");
                }
        ...
        public void Excluir(Guid id)
        {
            ...
            try
            {
                ...
                AddGuidParameter(deleteCommand, connection, "@Id", id);
        ...
        private static void LimparVinculosProdutos(DbConnection connection, DbTransaction transaction, Guid fornecedorId)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
        ...
        private Fornecedor? ObterUnico(string whereClause, object value)
        {
            using var connection = _connectionFactory();
            connection.Open();
            ...
            using (var command = connection.CreateCommand())
            {
                ...
                command.CommandText = $@"
                    {selectFirst} {FornecedorColumns}
                    FROM Fornecedores
                    WHERE {whereClause}
                    {limitOne};";
                if (value is Guid guidValue)
                {
                    AddGuidParameter(command, connection, "@Valor", guidValue);
                }
                else
                {
        ...
        private void PrepararEValidarFornecedor(Fornecedor fornecedor, DbConnection connection, DbTransaction? transaction)
        {
            ...
            fornecedor.PrazoMedioPagamentoDias = Math.Max(0, fornecedor.PrazoMedioPagamentoDias);
            ...
            var erroCnpj = CadastroValidationHelper.ValidarCpfOuCnpj(fornecedor.CNPJ, "CPF ou CNPJ", obrigatorio: false);
        ...
        private void ValidarDuplicidadeFornecedor(Fornecedor fornecedor, DbConnection connection, DbTransaction? transaction)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
                ...
                WHERE Id <> @Id
                  AND Ativo = 1;";
            AddGuidParameter(command, connection, "@Id", fornecedor.Id);
            ...
            while (reader.Read())
            {
                ...
                {
                    throw new InvalidOperationException("Ja existe um fornecedor ativo com este CPF ou CNPJ.");
                }
        ...
        private static void AddFornecedorParameters(DbCommand command, DbConnection connection, Fornecedor fornecedor)
        {
            AddGuidParameter(command, connection, "@Id", fornecedor.Id);
        ...
        private static void SalvarContatos(DbConnection connection, DbTransaction transaction, Fornecedor fornecedor)
        {
            foreach (var contato in fornecedor.Contatos ?? new List<ContatoFornecedor>())
            {
                ...
                AddGuidParameter(command, connection, "@Id", contato.Id);
                AddGuidParameter(command, connection, "@FornecedorId", fornecedor.Id);
        ...
        private static void ExcluirContatos(DbConnection connection, DbTransaction transaction, Guid fornecedorId)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
        ...
        private static void ExcluirProdutoFornecedores(DbConnection connection, DbTransaction transaction, Guid fornecedorId)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
        ...
        private static void CarregarContatos(DbConnection connection, IEnumerable<Fornecedor> fornecedores)
        {
        ...
        private static List<ContatoFornecedor> ObterContatos(DbConnection connection, Guid fornecedorId)
        {
        ...
        private static void VincularProdutosRelacionados(DbConnection connection, DbTransaction transaction, Fornecedor fornecedor)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
        ...
        private static Guid ReadGuid(DbDataReader reader, int index)
        {
            ...
            return !Guid.TryParse(Convert.ToString(raw), out var value)
                ? Guid.NewGuid()
                : value;
        }
        ...
        private static string? CriarSnapshot(Fornecedor? fornecedor)
        {
            return fornecedor == null
                ? null
                : $"RazaoSocial={fornecedor.RazaoSocial}; NomeFantasia={fornecedor.NomeFantasia}; Documento={fornecedor.CNPJ}; WhatsApp={fornecedor.WhatsAppVendedor}; PrazoMedioPagamento={fornecedor.PrazoMedioPagamentoDias}; CategoriaPreferencial={fornecedor.CategoriaPreferencial}; Ativo={fornecedor.Ativo}; Nota={fornecedor.Nota}; Contatos={fornecedor.Contatos.Count}";
        }
