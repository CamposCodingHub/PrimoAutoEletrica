...
namespace PrimoAutoEletrica.Repositories
{
    public sealed class ClienteRepository : IClienteRepository
    {
        ...
        private void PrepararEValidarCliente(Cliente cliente, DbConnection connection, DbTransaction? transaction)
        {
            cliente.Nome = cliente.Nome?.Trim() ?? string.Empty;
            cliente.TipoPessoa = NormalizarTipoPessoa(cliente.TipoPessoa, cliente.CPF);
        ...
        private void PrepararEValidarVeiculo(Veiculo veiculo, DbConnection connection, DbTransaction? transaction)
        {
            ...
            veiculo.BateriaInstalada = veiculo.BateriaInstalada?.Trim() ?? string.Empty;
            veiculo.BateriaMarca = veiculo.BateriaMarca?.Trim() ?? string.Empty;
            veiculo.BateriaAmperagem = veiculo.BateriaAmperagem?.Trim() ?? string.Empty;
            veiculo.Alternador = veiculo.Alternador?.Trim() ?? string.Empty;
            veiculo.MotorPartida = veiculo.MotorPartida?.Trim() ?? string.Empty;
            veiculo.TesteTensaoRepouso = veiculo.TesteTensaoRepouso?.Trim() ?? string.Empty;
            veiculo.TesteTensaoPartida = veiculo.TesteTensaoPartida?.Trim() ?? string.Empty;
            veiculo.TesteCargaAlternador = veiculo.TesteCargaAlternador?.Trim() ?? string.Empty;
            veiculo.CorrenteFuga = veiculo.CorrenteFuga?.Trim() ?? string.Empty;
            veiculo.EstadoAterramentos = veiculo.EstadoAterramentos?.Trim() ?? string.Empty;
            veiculo.ChicotesReparados = veiculo.ChicotesReparados?.Trim() ?? string.Empty;
            veiculo.FusiveisSubstituidos = veiculo.FusiveisSubstituidos?.Trim() ?? string.Empty;
            veiculo.RelesSubstituidos = veiculo.RelesSubstituidos?.Trim() ?? string.Empty;
            veiculo.LampadasSubstituidas = veiculo.LampadasSubstituidas?.Trim() ?? string.Empty;
            veiculo.AcessoriosInstalados = veiculo.AcessoriosInstalados?.Trim() ?? string.Empty;
            veiculo.ObservacoesTecnicasEletricas = veiculo.ObservacoesTecnicasEletricas?.Trim() ?? string.Empty;
            veiculo.FotosTecnicas = veiculo.FotosTecnicas?.Trim() ?? string.Empty;
        ...
        private void ValidarDuplicidadeCliente(Cliente cliente, DbConnection connection, DbTransaction? transaction)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
        ...
        private static bool ExistePlacaEmOutroVeiculo(Veiculo veiculo, DbConnection connection, DbTransaction? transaction)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
        ...
        private static Cliente MaterializarCliente(DbDataReader reader)
        {
            return new Cliente
            {
                Id = ReadGuid(reader, 0),
                Nome = ReadString(reader, 1),
                TipoPessoa = NormalizarTipoPessoa(ReadString(reader, 2), ReadString(reader, 3)),
                CPF = ReadString(reader, 3),
                RG = ReadString(reader, 4),
                DataNascimento = ReadNullableDate(reader, 5),
                Telefone = ReadString(reader, 6),
                WhatsApp = ReadString(reader, 7),
                Email = ReadString(reader, 8),
                CEP = ReadString(reader, 9),
                Rua = ReadString(reader, 10),
                Numero = ReadString(reader, 11),
                Bairro = ReadString(reader, 12),
                Cidade = ReadString(reader, 13),
                ...
                DataConsentimentoLGPD = ReadNullableDate(reader, 21),
                OrigemConsentimentoLGPD = ReadString(reader, 22),
                AutorizaContatoWhatsApp = ReadBool(reader, 23),
                Observacoes = ReadString(reader, 24),
                CaminhoDocumento = ReadString(reader, 25),
                CaminhoAssinatura = ReadString(reader, 26),
                DataCadastro = ReadDate(reader, 27, DateTime.Now),
                UltimaVisita = ReadNullableDate(reader, 28),
                ImagemUrl = ReadString(reader, 29)
            };
        }

        private static Veiculo MaterializarVeiculo(DbDataReader reader)
        {
            return new Veiculo
            {
                Id = ReadGuid(reader, 0),
                ...
                BateriaDataInstalacao = ReadNullableDate(reader, 31),
        ...
        private static void AddClienteParameters(DbCommand command, Cliente cliente)
        {
            command.Parameters.AddWithValue("@Id", cliente.Id.ToString());
            command.Parameters.AddWithValue("@Nome", ToDbNullableString(cliente.Nome));
            command.Parameters.AddWithValue("@TipoPessoa", NormalizarTipoPessoa(cliente.TipoPessoa, cliente.CPF));
        ...
        private static void AddVeiculoParameters(DbCommand command, Veiculo veiculo)
        {
            command.Parameters.AddWithValue("@Id", veiculo.Id.ToString());
        ...
        private void SalvarVeiculosDoCliente(
            ...
            DbTransaction? transaction)
        {
            var veiculosNormalizados = (veiculos ?? Enumerable.Empty<Veiculo>())
                .Where(veiculo => veiculo != null)
                .ToList();
            var veiculosExistentes = ObterVeiculoIdsDoCliente(clienteId, connection, transaction);
            var veiculosMantidos = new HashSet<Guid>();

            foreach (var veiculo in veiculosNormalizados)
            {
                veiculo.Id = veiculo.Id == Guid.Empty ? Guid.NewGuid() : veiculo.Id;
                veiculo.ClienteId = clienteId;
                veiculosMantidos.Add(veiculo.Id);
                SalvarVeiculo(veiculo, connection, transaction, replace: true);
            }

            foreach (var veiculoId in veiculosExistentes.Except(veiculosMantidos))
            {
                ExcluirVeiculoDoClienteSeSeguro(veiculoId, clienteId, connection, transaction);
            }
        }

        private static HashSet<Guid> ObterVeiculoIdsDoCliente(
            Guid clienteId,
            DbConnection connection,
            DbTransaction? transaction)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = "SELECT Id FROM Veiculos WHERE ClienteId = @ClienteId;";
            command.Parameters.AddWithValue("@ClienteId", clienteId.ToString());
        ...
        private static void ExcluirVeiculoDoClienteSeSeguro(
            Guid veiculoId,
            Guid clienteId,
            DbConnection connection,
            DbTransaction? transaction)
        {
            if (VeiculoPossuiReferenciasOperacionais(veiculoId, connection, transaction))
            {
                return;
            }
            ...
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = "DELETE FROM Veiculos WHERE Id = @Id AND ClienteId = @ClienteId;";
            command.Parameters.AddWithValue("@Id", veiculoId.ToString());
            command.Parameters.AddWithValue("@ClienteId", clienteId.ToString());
            command.ExecuteNonQuery();
        }

        private static bool VeiculoPossuiReferenciasOperacionais(
            Guid veiculoId,
            DbConnection connection,
            DbTransaction? transaction)
        {
            ...
            command.CommandText = @"
                SELECT
                    (SELECT COUNT(*) FROM OrdensServico WHERE VeiculoId = @VeiculoId) +
                    (SELECT COUNT(*) FROM Orcamentos WHERE VeiculoId = @VeiculoId);";
            command.Parameters.AddWithValue("@VeiculoId", veiculoId.ToString());
            ...
            if (replace)
            {
                command.CommandText = @"
                    ...
                    SET
                        ...
                        Renavam = @Renavam,
                        ImagemUrl = @ImagemUrl,
                        DocumentoImagemUrl = @DocumentoImagemUrl,
                        TipoVeiculo = @TipoVeiculo,
                        SistemaEletrico = @SistemaEletrico,
                        Motor = @Motor,
                        Combustivel = @Combustivel,
                        BateriaPrincipal = @BateriaPrincipal,
                        BateriaAuxiliar = @BateriaAuxiliar,
                        Alternador = @Alternador,
                        MotorPartida = @MotorPartida,
                        Quilometragem = @Quilometragem,
                        HistoricoTecnico = @HistoricoTecnico,
                        ObservacoesEletricasRecorrentes = @ObservacoesEletricasRecorrentes,
                        ProblemaRecorrente = @ProblemaRecorrente,
                        ObservacaoImportanteTecnico = @ObservacaoImportanteTecnico,
                        RetornoRecomendadoEm = @RetornoRecomendadoEm,
                        GarantiaValidaAte = @GarantiaValidaAte,
                        ProximaRevisaoEm = @ProximaRevisaoEm,
                        Observacoes = @Observacoes,
                        BateriaInstalada = @BateriaInstalada,
                        BateriaMarca = @BateriaMarca,
                        BateriaAmperagem = @BateriaAmperagem,
                        BateriaDataInstalacao = @BateriaDataInstalacao,
                        TesteTensaoRepouso = @TesteTensaoRepouso,
                        TesteTensaoPartida = @TesteTensaoPartida,
                        TesteCargaAlternador = @TesteCargaAlternador,
                        CorrenteFuga = @CorrenteFuga,
                        EstadoAterramentos = @EstadoAterramentos,
                        ChicotesReparados = @ChicotesReparados,
                        FusiveisSubstituidos = @FusiveisSubstituidos,
                        RelesSubstituidos = @RelesSubstituidos,
                        LampadasSubstituidas = @LampadasSubstituidas,
                        AcessoriosInstalados = @AcessoriosInstalados,
                        ObservacoesTecnicasEletricas = @ObservacoesTecnicasEletricas,
                        FotosTecnicas = @FotosTecnicas
                    WHERE Id = @Id;";
        ...
        private static string NormalizarTipoPessoa(string? tipoPessoa, string? documento)
        {
        ...
        private static string? CriarSnapshot(Cliente? cliente)
        {
            return cliente == null
                ? null
                : $"Nome={cliente.Nome}; TipoPessoa={cliente.TipoPessoa}; Documento={cliente.Documento}; Telefone={cliente.Telefone}; Ativo={cliente.Ativo}; Vip={cliente.ClienteVip}; LGPD={cliente.ConsentimentoLGPD}; WhatsApp={cliente.AutorizaContatoWhatsApp}; Veiculos={cliente.Veiculos.Count}";
        }
