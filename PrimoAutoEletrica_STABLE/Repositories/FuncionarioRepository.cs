...
namespace PrimoAutoEletrica.Repositories
{
    public sealed class FuncionarioRepository : IFuncionarioRepository
    {
        ...
        public FuncionarioRepository(Func<DbConnection> connectionFactory, LoggerService logger)
        {
            ...
            int funcionarioId)
        {
            ...
            funcionarioCommand.CommandText = $@"
                {selectPrefix} Nome, PerfilAcesso, Status, Ativo
                FROM Funcionarios
                WHERE Id = @Id
                {limitSuffix};";
        ...
        private static void ValidarUltimoAdministrador(DbConnection connection, DbTransaction transaction, int funcionarioId, string perfil, bool novoStatusAtivo)
        {
        ...
        private static void PreencherParametrosFuncionario(DbCommand command, Funcionario funcionario, bool incluirSenha)
        {
        ...
        private static Funcionario MaterializarFuncionario(DbDataReader reader)
        {
            return new Funcionario
            {
                ...
                DataUltimoLogin = ReadDateTime(reader, 13),
                ExigirTrocaSenha = ReadBool(reader, 14),
        ...
        {
            return $"Nome={funcionario.Nome}; CPF={funcionario.CPF}; Email={funcionario.Email}; Funcao={funcionario.Funcao}; Perfil={funcionario.PerfilAcesso}; Status={funcionario.Status}; Ativo={funcionario.Ativo}; Observacoes={(string.IsNullOrWhiteSpace(funcionario.Observacoes) ? "Nao" : "Sim")}; Foto={(string.IsNullOrWhiteSpace(funcionario.Foto) ? "Nao" : "Sim")}";
        }
        ...
        private static void PrepararEValidarFuncionario(Funcionario funcionario, int? ignorarFuncionarioId)
        {
            ...
            funcionario.Observacoes = funcionario.Observacoes?.Trim() ?? string.Empty;
