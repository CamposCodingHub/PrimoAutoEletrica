...
namespace PrimoAutoEletrica.Data.Repositories
{
    public class ImportacaoRepository
    {
        ...
        public List<NotaFiscalImportada> ObterHistoricoImportacoes(int limite = 50)
        {
            ...
            var command = connection.CreateCommand();
            command.CommandText = $@"
                SELECT {TopClause(connection, "@Limite")} n.Id, n.ChaveAcesso, n.Numero, n.Serie, n.DataEmissao, n.DataEntrada, n.ValorTotal, n.ValorProdutos,
                       n.Modelo, n.FornecedorNome, n.FornecedorCNPJ, n.Status, n.CaminhoArquivo, n.Erro, n.DataImportacao, n.UsuarioId, n.UsuarioNome,
                       ...
                       COALESCE((SELECT COUNT(*) FROM ImportacoesItens i WHERE i.ImportacaoId = n.Id AND i.Status = 'Erro'), 0)
                FROM ImportacoesNFe n
                ORDER BY DataImportacao DESC
                {LimitClause(connection, "@Limite")}
            ";
            ...
            while (reader.Read())
            {
                var nota = new NotaFiscalImportada
                {
                    ...
                    {
                        Nome = ReadString(reader, 9),
                        CNPJ = ReadString(reader, 10)
                    },
                    Status = Enum.Parse<StatusImportacaoNota>(ReadString(reader, 11)),
                    CaminhoArquivo = ReadString(reader, 12),
                    Erro = ReadString(reader, 13),
                    DataImportacao = ReadDate(reader, 14),
                    UsuarioId = ReadNullableGuid(reader, 15) ?? Guid.Empty,
                    UsuarioNome = ReadString(reader, 16),
                    QuantidadeProdutos = reader.IsDBNull(17) ? 0 : Convert.ToInt32(reader.GetValue(17)),
                    ProdutosNovos = reader.IsDBNull(18) ? 0 : Convert.ToInt32(reader.GetValue(18)),
                    ProdutosAtualizados = reader.IsDBNull(19) ? 0 : Convert.ToInt32(reader.GetValue(19)),
                    PossuiPendenciaProdutos = !reader.IsDBNull(20) && Convert.ToInt32(reader.GetValue(20)) > 0
                };
        ...
        public NotaFiscalImportada? ObterImportacaoPorId(Guid id)
        {
            ...
            NotaFiscalImportada? nota = null;
            using (var reader = command.ExecuteReader())
            {
                if (reader.Read())
                {
                    nota = new NotaFiscalImportada
                    {
                        Id = ReadGuid(reader, 0),
                        ChaveAcesso = ReadString(reader, 1),
                        Numero = ReadString(reader, 2),
            ...
            nota.QuantidadeProdutos = nota.Produtos.Count;
            nota.ProdutosNovos = nota.Produtos.Count(item => item.Status == StatusImportacao.Novo);
            nota.ProdutosAtualizados = nota.Produtos.Count(item => item.Status == StatusImportacao.Atualizado);
            nota.PossuiPendenciaProdutos = nota.Produtos.Any(item => item.Status == StatusImportacao.Erro);

            return nota;
        }

        private List<ProdutoImportado> ObterItensImportacao(Guid importacaoId, DbConnection connection)
        {
            ...
            while (reader.Read())
            {
                var item = new ProdutoImportado
                {
                    ...
                    MotivoIgnorado = ReadString(reader, 10),
                    ...
                    ProdutoSnapshotAnterior = ReadString(reader, 19),
                    ProdutoSnapshotPosterior = ReadString(reader, 20)
                };
        ...
        private static ImportacaoRollbackCabecalho? ObterCabecalhoRollback(DbConnection connection, DbTransaction transaction, Guid id)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = $@"
                SELECT {TopOne(connection)} Id, ChaveAcesso, Numero, Serie, FornecedorNome, FornecedorCNPJ, DataImportacao
                FROM ImportacoesNFe
                WHERE Id = @Id
                {LimitOne(connection)};";
            command.Parameters.AddWithValue("@Id", id.ToString());
        ...
        private static List<ImportacaoRollbackItem> ObterItensRollback(DbConnection connection, DbTransaction transaction, Guid importacaoId)
        {
            ...
            int limit)
        {
            ...
            command.Transaction = transaction;
            command.CommandText = $@"
                SELECT {TopClause(connection, "@Limit")}
                    Id,
                    Codigo,
                    Nome,
                    Descricao,
        ...
        private static bool TabelaExiste(DbConnection connection, DbTransaction transaction, string tabela)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = IsSqlServerConnection(connection)
        ...
        private static string TopClause(DbConnection connection, string parameterName)
        {
            return IsSqlServerConnection(connection) ? $"TOP ({parameterName})" : string.Empty;
        }

        private static string LimitClause(DbConnection connection, string parameterName)
        {
            return IsSqlServerConnection(connection) ? string.Empty : $"LIMIT {parameterName}";
        }
        ...
        private static DateTime ReadDate(DbDataReader reader, int index)
        {
            ...
            return DateTime.TryParse(Convert.ToString(value, CultureInfo.InvariantCulture), CultureInfo.InvariantCulture, DateTimeStyles.RoundtripKind, out var parsed)
                ? parsed
                : DateTime.MinValue;
        }
