...
{
    public static class VeiculoMediaService
    {
        ...
        {
            var directory = Path.Combine(App.RuntimeAppDataPath, "Media", "Veiculos");
        ...
        public static string PersistSelectedImage(string sourcePath, Guid veiculoId, string descricaoVeiculo, string categoria)
        {
            if (string.IsNullOrWhiteSpace(sourcePath) || !File.Exists(sourcePath))
            {
                throw new FileNotFoundException("A imagem selecionada para o veiculo nao foi encontrada.", sourcePath);
            }
            ...
            var safeName = SanitizeFileSegment(descricaoVeiculo);
            var safeCategory = SanitizeFileSegment(categoria);
            var destinationPath = Path.Combine(
                destinationDirectory,
                $"{DateTime.Now:yyyyMMddHHmmss}_{safeCategory}_{safeName}_{veiculoId:N}{extension}");
        ...
        {
            var text = string.IsNullOrWhiteSpace(value) ? "veiculo" : value.Trim();
