...
namespace PrimoAutoEletrica.Services
{
    public class OrcamentoPdfService
    {
        ...
        public void GerarPdfOrcamento(Orcamento orcamento, string caminhoArquivo)
        {
            ...
            DrawTextSection("Diagnostico tecnico", Safe(orcamento.Diagnostico, "Sem diagnostico informado."));
        ...
        private static Veiculo? ResolveVeiculo(Orcamento orcamento, Cliente? cliente)
        {
            if (orcamento.Veiculo != null)
            {
                return orcamento.Veiculo;
            }

            if (orcamento.VeiculoId.HasValue)
            {
                var veiculoDoCliente = cliente?.Veiculos.FirstOrDefault(veiculo => veiculo.Id == orcamento.VeiculoId.Value);
        ...
        private static string BuildVeiculo(Veiculo? veiculo)
        {
            if (veiculo == null)
            {
                return "Nao informado";
            ...
            return string.IsNullOrWhiteSpace(descricao) ? "Nao informado" : descricao.Trim();
        }
