...
{
    public static class ClienteMediaService
    {
        ...
        {
            var directory = Path.Combine(App.RuntimeAppDataPath, "Media", "Clientes");
        ...
        public static string PersistSelectedImage(string sourcePath, Guid clienteId, string nomeCliente)
        {
            if (string.IsNullOrWhiteSpace(sourcePath) || !File.Exists(sourcePath))
            {
                throw new FileNotFoundException("A imagem selecionada para o cliente nao foi encontrada.", sourcePath);
            }
            ...
            var safeName = SanitizeFileSegment(nomeCliente);
            var destinationPath = Path.Combine(
                destinationDirectory,
                $"{DateTime.Now:yyyyMMddHHmmss}_{safeName}_{clienteId:N}{extension}");
        ...
        public static void DeleteManagedImageIfOwned(string? persistedPath)
        {
            ...
            {
                // Nao bloqueia o fluxo do cadastro se o arquivo ja tiver sido removido externamente.
            }
        }
        ...
        {
            var text = string.IsNullOrWhiteSpace(value) ? "cliente" : value.Trim();
