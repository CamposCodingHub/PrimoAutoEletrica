...
namespace PrimoAutoEletrica.Services
{
    ...
    public sealed class OperationalWorkflowTestService
    {
        ...
        public WorkflowTestRunResult Run()
        {
            ...
            try
            {
                ...
                RunCheck(result, "Ambiente:Isolado", () =>
                {
                    ...
                    var isSqlServerRuntime = IsSqlServerRuntime();
                    if (isSqlServerRuntime)
                    {
                        if (string.IsNullOrWhiteSpace(App.Database.DatabasePath))
                        {
                            throw new InvalidOperationException("Nome do banco SQL Server isolado nao foi configurado.");
                        }

                        if (!App.RuntimeAppDataPath.Contains("AutomatedTests", StringComparison.OrdinalIgnoreCase) &&
                            !App.RuntimeAppDataPath.Contains("TestResults", StringComparison.OrdinalIgnoreCase))
                        {
                            throw new InvalidOperationException("Workflow test SQL Server nao esta usando app-data isolado de automacao.");
                        }
                ...
                RunCheck(result, "Agendamentos:TimelineEAlertas", () =>
                {
                    ...
                    var inicioAtrasado = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromMinutes(30));
                    var terminoAtrasado = inicioAtrasado.AddHours(1);
                    var inicioProlongado = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromHours(1));
                    var terminoProlongado = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromMinutes(20));
                    var checkInProlongado = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromMinutes(40));
                    var checkInPronto = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromMinutes(50));
                    var checkOutPronto = CreateAgendaTimeInCurrentDay(agora, TimeSpan.FromMinutes(10));
                    ...
                    agendamentoAtrasado.HoraInicio = inicioAtrasado;
                    agendamentoAtrasado.HoraTermino = terminoAtrasado;
                    ...
                    agendamentoProlongado.HoraInicio = inicioProlongado;
                    agendamentoProlongado.HoraTermino = terminoProlongado;
                    agendamentoProlongado.CheckIn = checkInProlongado;
                    ...
                    agendamentoPronto.CheckIn = checkInPronto;
                    agendamentoPronto.CheckOut = checkOutPronto;
                ...
                RunCheck(result, "Backup:NomePadronizado", () =>
                {
                    ...
                    var extensaoEsperada = IsSqlServerRuntime() ? ".bak" : ".db";
                    ...
                    if (!fileName.EndsWith(extensaoEsperada, StringComparison.OrdinalIgnoreCase))
                    {
                        throw new InvalidOperationException($"Backup nao tem extensao {extensaoEsperada}: {fileName}");
                    }
        ...
        private static DateTime CreateAgendaTimeInCurrentDay(DateTime now, TimeSpan offsetFromNow)
        {
            var currentTime = now.TimeOfDay;
            var targetTime = currentTime > offsetFromNow
                ? currentTime - offsetFromNow
                : TimeSpan.Zero;

            return now.Date.Add(targetTime);
        }
            ...
            string descricaoServico = "Mao de obra tecnica")
        {
            ...
            return new Orcamento
            {
                ...
                Diagnostico = "Diagnostico operacional gerado pelo workflow test.",
        ...
        private int CountAuditLogs(DateTime startedAt, IReadOnlyCollection<string> categorias)
        {
            ...
            var isSqlServerRuntime = IsSqlServerRuntime();
            command.CommandText = isSqlServerRuntime
                ? $@"
                SELECT COUNT(*)
                FROM AuditLogs
                WHERE DataHora >= @StartedAt
                ...
                WHERE datetime(DataHora) >= datetime(@StartedAt)
                  AND Categoria IN ({string.Join(", ", parametros)});";
            command.Parameters.AddWithValue("@StartedAt", isSqlServerRuntime ? startedAt : startedAt.ToString("yyyy-MM-dd HH:mm:ss"));
        ...
        private static bool IsSqlServerRuntime()
        {
            return string.Equals(App.Database.RuntimeProvider, "SqlServer", StringComparison.OrdinalIgnoreCase);
        }
        ...
        private string PersistReport(WorkflowTestRunResult result)
        {
            ...
            var path = Path.Combine(baseDirectory, $"workflow-test-{DateTime.Now:yyyy-MM-dd-HH-mm-ss-fff}-p{Environment.ProcessId}.txt");
