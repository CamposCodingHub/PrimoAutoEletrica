...
namespace PrimoAutoEletrica.Views
{
    public partial class VisualizarFornecedorWindow : Window
    {
        ...
        private void CarregarDadosFornecedor()
        {
            ...
            PrazoMedioPagamentoText.Text = _fornecedor.PrazoMedioPagamentoDias > 0 ? $"{_fornecedor.PrazoMedioPagamentoDias} dia(s)" : "Nao informado";
            ...
            ContasPagarFornecedorResumoText.Text =
                $"{insights.ContasPagarVinculadas} titulo(s) | aberto R$ {insights.ValorContasPagarAberto:F2} | total R$ {insights.ValorContasPagarTotal:F2}";
            ContasPagarFornecedorItemsControl.ItemsSource = insights.ContasPagarRecentes.Any()
                ? insights.ContasPagarRecentes.Select(item => new
                {
                    item.Descricao,
                    item.Valor,
                    Status = $"{item.Status} | {Exibir(item.Categoria)}",
                    Vencimento = item.DataVencimento == DateTime.MinValue
                        ? "Vencimento nao informado"
                        : $"Vence em {item.DataVencimento:dd/MM/yyyy}"
                }).ToList()
                : new[]
                {
                    new
                    {
                        Descricao = "Sem contas vinculadas",
                        Valor = 0m,
                        Status = "Nenhum titulo financeiro encontrado",
                        Vencimento = "Financeiro sem pendencias para este fornecedor"
                    }
                };
