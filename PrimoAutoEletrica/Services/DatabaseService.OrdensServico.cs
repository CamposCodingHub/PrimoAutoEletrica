...
namespace PrimoAutoEletrica.Services
{
    ...
    {
        private void InitializeVeiculosSchema(SqliteConnection connection)
        {
        ...
        private static bool VeiculosSchemaPrecisaRecriar(SqliteConnection connection)
        {
        ...
        private void InitializeOrdensServicoSchema(SqliteConnection connection)
        {
        ...
        private OrdemServico MapOrdemServico(SqliteDataReader reader)
        {
            return new OrdemServico
            {
                ...
                AprovadaCliente = ReadBool(reader, 16),
                MetodoAprovacao = ReadString(reader, 17),
                DataAbertura = ReadDateTime(reader, 18) ?? DateTime.Now,
                DataPrevisao = ReadDateTime(reader, 19),
                DataAprovacao = ReadDateTime(reader, 20),
                DataInicio = ReadDateTime(reader, 21),
                DataConclusao = ReadDateTime(reader, 22),
                DataEntrega = ReadDateTime(reader, 23),
                ValorMaoObra = ReadDecimal(reader, 24),
                Desconto = ReadDecimal(reader, 25),
                Ativo = ReadBool(reader, 26)
            };
        }
