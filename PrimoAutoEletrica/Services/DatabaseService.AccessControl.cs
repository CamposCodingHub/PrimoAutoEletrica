...
namespace PrimoAutoEletrica.Services
{
    public partial class DatabaseService
    {
        ...
        private void InitializeAccessControlSchema(SqliteConnection connection)
        {
        ...
        private static void HardenAccessControlIndexes(SqliteConnection connection)
        {
        ...
        public bool? ObterPermissaoPorPerfilECodigo(string perfilAcesso, string codigoPermissao)
        {
            ...
            using var command = connection.CreateCommand();
            command.CommandText = @"
                SELECT pa.Ativo, pp.Ativa, pp.Concedida, p.Ativo
                FROM PerfisAcesso pa
                INNER JOIN PerfilPermissoes pp ON pp.PerfilId = pa.Id
                INNER JOIN Permissoes p ON p.Id = pp.PermissaoId
                WHERE lower(pa.Nome) = lower(@Perfil)
        ...
        public int CriarPerfilAcesso(PerfilAcesso perfil, IEnumerable<int> permissaoIds, string operador)
        {
            ...
            {
                using var insertPermissao = connection.CreateCommand();
                insertPermissao.Transaction = transaction;
                insertPermissao.CommandText = @"
                    ...
                    DO UPDATE SET Concedida = 1,
                                  ...
                                  RevogadaPor = NULL,
                                  Ativa = 1;";
                insertPermissao.Parameters.AddWithValue("@PerfilId", perfilId);
                insertPermissao.Parameters.AddWithValue("@PermissaoId", permissaoId);
                insertPermissao.Parameters.AddWithValue("@DataConcessao", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                insertPermissao.Parameters.AddWithValue("@ConcedidaPor", string.IsNullOrWhiteSpace(operador) ? "Sistema" : operador.Trim());
                insertPermissao.ExecuteNonQuery();
            }
            ...
            foreach (var permissaoId in permissoesAtualizadas)
            {
                ...
                upsert.CommandText = @"
                    ...
                    DO UPDATE SET Concedida = 1,
                                  ...
                                  RevogadaPor = NULL,
                                  Ativa = 1;";
                upsert.Parameters.AddWithValue("@PerfilId", perfil.Id);
                upsert.Parameters.AddWithValue("@PermissaoId", permissaoId);
                upsert.Parameters.AddWithValue("@DataConcessao", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                upsert.Parameters.AddWithValue("@ConcedidaPor", operadorNormalizado);
        ...
        private static Permissao? ObterPermissaoPorId(SqliteConnection connection, int permissaoId, SqliteTransaction? transaction = null)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
                SELECT Id, Nome, Descricao, Modulo, Acao, Codigo, Ativo, Essencial, OrdemExibicao
                FROM Permissoes
                WHERE Id = @Id
                LIMIT 1;";
            ...
            return new Permissao
            {
                ...
                Essencial = reader.GetInt32(7) == 1,
                OrdemExibicao = reader.GetInt32(8)
            };
        }

        private static PerfilAcesso? ObterPerfilAcessoPorId(SqliteConnection connection, int perfilId, SqliteTransaction? transaction = null)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
                SELECT Id, Nome, Descricao, NivelHierarquico, Ativo, PodeDeletar, OrdemExibicao
                FROM PerfisAcesso
                WHERE Id = @Id
                LIMIT 1;";
            ...
            return new PerfilAcesso
            {
                ...
                PodeDeletar = reader.GetInt32(5) == 1,
                OrdemExibicao = reader.GetInt32(6)
            };
        }

        private static List<int> ObterPermissoesAtivasDoPerfil(SqliteConnection connection, int perfilId, SqliteTransaction? transaction = null)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
        ...
        private void SeedPermissoes(SqliteConnection connection)
        {
            var permissoes = new[]
            {
        ...
        private void SeedPerfis(SqliteConnection connection)
        {
            var perfis = new[]
            {
        ...
        private void VincularPermissoesPadrao(SqliteConnection connection, PerfilSeed perfil)
        {
            ...
            var permissaoIds = ObterIdsPermissoes(connection, perfil.Modulos, somenteEssenciais: true);
            foreach (var permissaoId in permissaoIds)
            {
                using var command = connection.CreateCommand();
                command.CommandText = @"
                    INSERT INTO PerfilPermissoes (PerfilId, PermissaoId, Concedida, DataConcessao, ConcedidaPor, Ativa)
                    VALUES (@PerfilId, @PermissaoId, 1, @DataConcessao, 'Sistema', 1)
                    ON CONFLICT(PerfilId, PermissaoId)
                    DO UPDATE SET Concedida = 1,
                                  Ativa = 1,
                                  DataRevogacao = NULL,
        ...
        private static int ObterIdPerfil(SqliteConnection connection, string nome)
        {
            using var command = connection.CreateCommand();
            command.CommandText = "SELECT Id FROM PerfisAcesso WHERE Nome = @Nome LIMIT 1;";
            command.Parameters.AddWithValue("@Nome", nome);
            return Convert.ToInt32(command.ExecuteScalar() ?? 0);
        }

        private static List<int> ObterIdsPermissoes(SqliteConnection connection, IReadOnlyCollection<string> modulos, bool somenteEssenciais = false)
        {
            using var command = connection.CreateCommand();
            if (modulos.Contains("*"))
            {
        ...
        private static string ReadAccessString(SqliteDataReader reader, int ordinal)
        {
            return reader.IsDBNull(ordinal) ? string.Empty : reader.GetString(ordinal);
        }

        private static bool ReadAccessBool(SqliteDataReader reader, int ordinal)
        {
            return !reader.IsDBNull(ordinal) && Convert.ToInt32(reader.GetValue(ordinal)) == 1;
        }

        private static DateTime ReadAccessDate(SqliteDataReader reader, int ordinal, DateTime fallback)
        {
            return DateTime.TryParse(ReadAccessString(reader, ordinal), out var date) ? date : fallback;
        }

        private static DateTime? ReadAccessNullableDate(SqliteDataReader reader, int ordinal)
        {
            return DateTime.TryParse(ReadAccessString(reader, ordinal), out var date) ? date : null;
        }
    }
}
