...
namespace PrimoAutoEletrica.Repositories
{
    public sealed class OrdemServicoRepository : IOrdemServicoRepository
    {
        ...
        public OrdemServicoRepository(Func<SqliteConnection> connectionFactory, LoggerService logger)
        {
        ...
        public string GerarProximoNumero()
        {
            ...
            var prefixo = $"OS-{DateTime.Now:yyyy}-";
            ...
            return $"{prefixo}{(sequencia + 1):0000}";
        }
        ...
        private static void AddOrdemParameters(SqliteCommand command, OrdemServico ordem)
        {
            ...
            command.Parameters.AddWithValue("@AgendamentoId", ordem.AgendamentoId?.ToString() ?? (object)DBNull.Value);
            ...
            command.Parameters.AddWithValue("@OrcamentoId", ordem.OrcamentoId?.ToString() ?? (object)DBNull.Value);
            ...
            SqliteTransaction transaction)
        {
            using var selectCommand = connection.CreateCommand();
            selectCommand.Transaction = transaction;
            selectCommand.CommandText = "SELECT Numero FROM OrdensServico WHERE Id = @Id LIMIT 1;";
            selectCommand.Parameters.AddWithValue("@Id", id.ToString());
        ...
        private static OrdemServico MapOrdemServico(SqliteDataReader reader)
        {
            var ordem = new OrdemServico
            {
                ...
                AssinaturaClienteUrl = ReadString(reader, 25),
                AprovadaCliente = ReadBool(reader, 26),
                MetodoAprovacao = ReadString(reader, 27),
                DataAbertura = ReadDateTime(reader, 28) ?? DateTime.Now,
                DataPrevisao = ReadDateTime(reader, 29),
                DataAprovacao = ReadDateTime(reader, 30),
                DataInicio = ReadDateTime(reader, 31),
                DataConclusao = ReadDateTime(reader, 32),
                DataEntrega = ReadDateTime(reader, 33),
                GarantiaValidaAte = ReadDateTime(reader, 34),
                TempoPrevistoMinutos = ReadInt32(reader, 35),
                TempoRealMinutos = ReadInt32(reader, 36),
                OrcamentoId = ReadNullableGuid(reader, 37),
                ValorMaoObra = ReadDecimal(reader, 38),
                Desconto = ReadDecimal(reader, 39),
                Ativo = ReadBool(reader, 40)
            };
        ...
        private void SafeRollback(SqliteTransaction transaction, string context)
        {
            try
            {
