...
namespace PrimoAutoEletrica.UserControls
{
    public partial class ImportarNFeControl : UserControl
    {
        ...
        private void CarregarHistoricoImportacoes()
        {
            ...
            try
            {
                ...
                foreach (var notaResumo in _importacaoRepository.ObterHistoricoImportacoes(120))
                {
                    ...
                    {
                        Id = notaResumo.Id,
                        DataImportacao = notaResumo.DataImportacao,
                        Fornecedor = TextoOuPadrao(notaResumo.Fornecedor.Nome),
                        CnpjFornecedor = TextoOuPadrao(notaResumo.Fornecedor.CNPJ),
                        NumeroNota = TextoOuPadrao(notaResumo.Numero),
                        Serie = TextoOuPadrao(notaResumo.Serie),
                        ChaveAcesso = TextoOuPadrao(notaResumo.ChaveAcesso),
                        QuantidadeProdutos = notaResumo.QuantidadeProdutos,
                        ProdutosNovos = notaResumo.ProdutosNovos,
                        ProdutosAtualizados = notaResumo.ProdutosAtualizados,
                        ValorTotal = notaResumo.ValorTotal,
                        Status = FormatarStatus(notaResumo.Status),
                        Usuario = string.IsNullOrWhiteSpace(notaResumo.UsuarioNome) ? "Sistema" : notaResumo.UsuarioNome.Trim(),
                        TemPendencia = notaResumo.Status is StatusImportacaoNota.Pendente or StatusImportacaoNota.Parcial or StatusImportacaoNota.Erro
                            || notaResumo.PossuiPendenciaProdutos,
                        Nota = notaResumo,
        ...
        private void VisualizarImportacaoButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            GarantirDetalhesDaImportacao(item);
            var nota = item.Nota;
            var detalhes =
                $"Fornecedor: {item.Fornecedor}\n" +
                $"CNPJ: {item.CnpjFornecedor}\n" +
        ...
        private void GarantirDetalhesDaImportacao(ImportacaoNFeGridItem item)
        {
            if (item.DetalhesCarregados || _importacaoRepository == null)
            {
                return;
            }

            var notaCompleta = _importacaoRepository.ObterImportacaoPorId(item.Id);
            if (notaCompleta != null)
            {
                item.Nota = notaCompleta;
            }
        ...
        private sealed class ImportacaoNFeGridItem
        {
            ...
            public NotaFiscalImportada Nota { get; set; } = new();
            public bool DetalhesCarregados { get; set; }
        }
    }
}
