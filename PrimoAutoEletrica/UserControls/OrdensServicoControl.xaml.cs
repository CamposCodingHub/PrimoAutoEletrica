    ...
    public partial class OrdensServicoControl : UserControl
    {
        ...
        private void AplicarFiltros(Guid? ordemPreferida = null)
        {
            ...
            if (!string.Equals(status, "Todos os status", StringComparison.OrdinalIgnoreCase))
                consulta = consulta.Where(o => string.Equals(o.Status, status, StringComparison.OrdinalIgnoreCase));
        ...
        private void AtualizarIndicadores()
        {
            ...
            AguardandoAprovacaoText.Text = abertas.Count(o => o.Status == "Aguardando aprovacao").ToString();
            AguardandoPecaText.Text = abertas.Count(o => o.Status == "Aguardando peca").ToString();
            ProntasText.Text = abertas.Count(o => o.Status == "Pronta para entrega").ToString();
        ...
        private static string? ObterProximoStatus(OrdemServico ordem)
        {
            return ordem.Status switch
            {
                ...
                "Aguardando aprovacao" => ordem.AprovadaCliente ? "Aprovada" : null,
                "Aprovada" => "Em diagnostico",
                "Em diagnostico" => "Em execucao",
                "Aguardando peca" => "Em execucao",
                "Em execucao" => "Pronta para entrega",
